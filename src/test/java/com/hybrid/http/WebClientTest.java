...
//		GET();
	...
	static void POST() throws UnknownHostException, IOException {
		...
		Scanner respinse= new Scanner(sok.getInputStream());
		
		/*
		 * Request 전문 송신
		 */
		String body = "name=" + URLEncoder.encode("홀길동", "utf-8")
		...
		request.println("Content-Type:application/x-www-urlencode");
		request.println();
		 ...
		 * Response 전문 송신
		 */
		while(respinse.hasNextLine()) {
			String line = respinse.nextLine();
